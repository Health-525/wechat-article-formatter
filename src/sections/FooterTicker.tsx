...
export default function FooterTicker() {
  ...
  const startLineCycleRef = useRef<((state: LineState, delay: number) => void) | undefined>(undefined);
    ...
    const timeout = setTimeout(() => {
      ...
      const holdTimeout = setTimeout(() => {
        ...
        const pauseTimeout = setTimeout(() => {
          startLineCycleRef.current?.(state, 0);
        }, 2000);
  ...
  // Keep the latest callback available for recursive setTimeout calls
  useEffect(() => {
    startLineCycleRef.current = startLineCycle;
  }, [startLineCycle]);
  ...
  return (
    ...
    >
      ...
      {footerConfig.linkColumns.length > 0 && (
        <div
          className="footer-grid mx-auto grid grid-cols-2 gap-16"
