// 智思手记 —— 为 AI / 技术学习者设计的主题。
// 理性、有序、代码友好，但保留一点温度：暖灰底 + 深青强调，像研究员的笔记本。
...
export function strong(text: string): string {
  const t = DESIGN_TOKENS
  return `<strong style="color:${t.title};font-weight:700;letter-spacing:0.3px;">${span(text)}</strong>`
}

export function inlineCode(text: string): string {
  const t = DESIGN_TOKENS
  return `<span style="background:${t.paper};color:${t.primary};padding:1px 5px;border-radius:4px;font-family:${t.monoStack};font-size:0.86em;font-weight:500;border:1px solid rgba(46,107,117,0.15);letter-spacing:0.2px;">${span(text)}</span>`
}
...
export function codeBlock(language: string, code: string): string {
  ...
  return `
<section style="margin:0 0 28px;border-radius:10px;overflow:hidden;background:${t.codeBg};border:1px solid rgba(236,239,241,0.08);box-shadow:0 8px 24px -10px rgba(30,46,51,0.22);">
  <section style="display:flex;align-items:center;padding:9px 16px;background:${t.codeTitleBg};border-bottom:1px solid rgba(236,239,241,0.06);">
    <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${t.muted};margin-right:6px;font-size:0;line-height:0;overflow:hidden;">${span("")}</span>
    <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${t.secondary};margin-right:6px;font-size:0;line-height:0;overflow:hidden;">${span("")}</span>
...
<section style="margin:32px 0;">
  <img src="${src}" alt="${alt || ""}" style="width:100%;max-width:100%;height:auto;display:block;border-radius:10px;box-shadow:0 10px 28px -12px rgba(30,46,51,0.16);">
  ${caption}
</section>`
}
...
export function onelinerCard(children: string): string {
  const t = DESIGN_TOKENS
  return `
<section style="margin:0 0 32px;padding:20px;background:${t.paper};border-left:3px solid ${t.primary};">
  <p style="font-family:${t.titleFont};font-size:15px;color:${t.title};margin:0;line-height:1.7;">${children}</p>
</section>`
}
...
export function subheading(title: string): string {
  const t = DESIGN_TOKENS
  return `
<section style="margin:36px 0 20px;padding:3px 0 3px 14px;border-left:3px solid ${t.primary};">
