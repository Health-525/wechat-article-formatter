...
describe("mathRenderer", () => {
  ...
  it("renders block math to HTML with katex-display class", () => {
    const { prepared, snippets } = extractMath("$$x^2$$")
  ...
  it("injects KaTeX CSS only when math is present", () => {
    const withMath = renderMarkdownToHtml("$x^2$", "minimal-white")
    expect(withMath).toContain("<style>")
    expect(withMath).toContain("@font-face")
